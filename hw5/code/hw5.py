...
def psf2otf(psf: np.ndarray, shape: Tuple[int, int]) -> np.ndarray:
    ...
    psf_padded[:psf_bottom_half, shape[1] - psf_left_half :] = psf[
        psf_top_half:, :psf_left_half
    ]
    psf_padded[shape[0] - psf_top_half :, :psf_right_half] = psf[
        :psf_top_half, psf_left_half:
    ]
    psf_padded[shape[0] - psf_top_half :, shape[1] - psf_left_half :] = psf[
        :psf_top_half, :psf_left_half
    ]
